...
const double magic_constant = 17;
const int iter = 100;
int grid[3][3][3][3];
int gg[3][3];
int tur = -1;
...
int monte_karlo(int last, int move) {
    int ans = check(gg, tur);
    if (ans != -1) {
        return ans - 1;
    }
    ...
    ans = monte_karlo((x % 3) * 3 + (y % 3), move ^ 1);
    gg[x / 3][y / 3] = ls2;
...
*/
    ...
    int ans;
    vector<int> sum(n, 0), cnt(n, 0), pr(n, -1);
    int c = n * 10;
    for (int i = 0; i < n; i++) {
        ...
        for (int j = 0; j < 10; j++) {
            sum[i] += monte_karlo(((nex / 9) % 3) * 3 + (nex % 9) % 3, tur ^ 1);
            cnt[i]++;
        }
    ...
    for (int _ = 0; _ < iter; _++) {
        ...
        for (int i : st) {
            ...
            grid[x / 3][y / 3][x % 3][y % 3] = t;
            gg[x / 3][y / 3] = check(grid[x / 3][y / 3], tur);
            t ^= 1;
        }
        int nex = next_move((a[pos].first % 3) * 3 + (a[pos].second % 3));
        if (nex == -1) {
            int res = monte_karlo((a[pos].first % 3) * 3 + (a[pos].second % 3), t);
            while (pos != -1) {
                sum[pos] += res;
                cnt[pos]++;
                pos = pr[pos];
            ...
            cnt.push_back(0);
            pr.push_back(pos);
            int res = monte_karlo((x % 3) * 3 + (y % 3), t ^ 1);
            sum.back() += res;
            cnt.back()++;
            while (pos != -1) {
                sum[pos] += res;
